...
last_updated: "2026-08-04T21:26:42.402Z"
last_activity: 2026-08-04
last_activity_desc: "04-09 executed - THE PHASE GATE. Every gate this phase bought was aimed at and made to fail on its own label: 19 mutations declared, 19 executed, 17 of 17 CAUGHT with 2 controls green, run in a git-archive export of HEAD with its OWN node_modules so the working tree and .next could not be touched at all. A CAUGHT required three things together - a non-zero exit captured straight from spawnSync and never through a pipe, the expected label on a FAIL line, and a file that still parses - and restore was an in-memory byte snapshot, never git checkout. All three runner branches were smoke-tested live first (baseline green, SURVIVED live, ANCHOR-DEFECT refusing a 264-occurrence anchor with the file byte-identical). The id gate is proved to cover the ids THIS PHASE authored rather than only the 538 present when it was seeded: a LATE 04-07 id and an EARLY 04-01 id were re-pointed and both failed on their own names, a partial tip-only and a partial example-only edit under live ids both failed, an id added without regenerating the fixture failed as an unrecorded addition, and a retired id put back into its own bank failed on rule 4. INDEPENDENT COMPLETENESS, asserted from outside the script that owns it: 651 ids emitted by the banks against 651 in the fixture, AGREE EXACTLY, computed without importing verify-id-stability's logic - and two of MY OWN bugs had to be fixed before it agreed (the accessors take two arguments, not a composite key; scenario grammar composes its ids already while phrases and vocabulary carry bare slugs), which proved the DISAGREE branch live twice before its AGREE was believed. Derivation control re-proved: one emptied entry takes 53/53 to 52/53, puts the pair in pendingPairs, empties its summary and flips the scenario incomplete, with no second edit. THE READER PASS found NO near-duplicate exercise and rewrote nothing; same-world highest is 0.286 on phrases and 0.400 on terms with NOTHING at or above 0.50 anywhere, and the rise from 03-11's 0.14/0.00 is mostly POOL SIZE - the world went from 30 phrases and 40 cards to 84 and 112. A METHOD FINDING: a conventional stoplist scored catch-up against catch-on at J=1.000 because both collapse to catch, so A PARTICLE IS NOT A STOPWORD IN THIS WORLD and any future scan must keep particles as content words. Four reader findings judged and none fixed (I'm afraid on four surfaces but taught on only one; pinch-of-salt and face-value sharing a skeleton while teaching near-opposites; keep-someone-in-the-loop against fill-someone-in at 0.400; and native/register's briefing naming THREE of its own cards in one tip, which is the card half of WINDOWS 57's phrase half and strengthens the case for widening that gate). One scan flag was a FALSE POSITIVE and the reader overruled it - phrasal-verbs' catch on is 04-01's deliberate polysemy split and the phrase's tip names the card's sense by contrast. THE CORPUS FIGURE IS SETTLED by measurement at four commits: 4,410 at 33f082b which is 04-07's OWN final commit, 4,440 at HEAD, so 04-08 is right and 04-07's 4,523/4,565 does not reproduce - there are no 155 missing fields. The cause is REMOVED rather than documented: 04-05's broken corpus.mts, still imported by scan.mts and probe.mts and silently dropping 492 of 4,440 fields, now THROWS on evaluation naming its replacement, verified by running scan.mts and watching it exit 1. CONT-04 IS MARKED [~] MET-WITH-A-STATED-LIMITATION AND DELIBERATELY NOT [x]: content is complete, derived and gated (84 phrases, 112 cards, 20 grammar questions from 30/40/5; 155 ids added, 15 retired every one with a reason, ZERO re-pointed; 53/53 pairs, 0 pending, asserted) but THE BROWSER PASS WAS NOT PERFORMED and ten items are named unobserved rather than rounded up, including the batched recall deck which is this phase's one component change and has never been rendered, /review above sixteen due, ticking a rehearsal move, and ANYTHING AT ALL ON A PHONE. Two items at the top of that checklist came from the real beta user and outrank every internal check - whether tapping a drop-down blank still zooms the page on mobile (f164ec5, never confirmed on a device) and whether the CELPIP nav link (d06c219) renders and routes on a phone. The annotation also NAMES more reading as a deliberate deferral the user declined knowingly at 04-08, because D-01's own wording asked for it and this phase did not deliver it, and says why native/pronunciation is deliberately the smallest of the five. Ledger 62 to 66 added: the unperformed browser pass, option D declined by name, the reader pass, the corpus settlement, and THE state.* DEFECT CONSOLIDATED IN ONE PLACE WITH ITS ROOT CAUSE after SEVENTEEN consecutive occurrences. Harnesses all at baseline - 14,577 and 2,021 and 25,647 and 309 and 173 and 24 and 43/648/50; tsc, lint and build exit 0 from the committed tree after a .next wipe with the bundle grepped for phase content and for zero mutation artefacts; dependencies 11 + 11 with zero packages installed all phase; payload 205,174 B / 19.6 percent over 752 ids against 03-11's 143,830 B / 13.7 percent; port 3000 free before and after. WINDOWS 45/60 RECURRED FOR A SEVENTEENTH CONSECUTIVE PLAN: advance-plan and record-session BOTH reset total_phases 6 to 5, record-session wrote stopped_at unquoted, Current focus was never advanced, roadmap.update-plan-progress reported complete false again, and all five of my decision rows were stamped [Phase ?]. All hand-corrected AFTER the last verb; the 115 pre-existing [Phase ?] rows counted before and after and untouched. PHASE 4 IS COMPLETE, 9 of 9 plans."
progress:
  total_phases: 6
  completed_phases: 5
  total_plans: 45
