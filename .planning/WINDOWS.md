...
fixed_count: 20
total_count: 80
last_updated: 2026-08-05T06:22:54.175Z
---
...
| 78 | 04.1 | deviation | src/lib/content/core-vocabulary.ts |  | 04.1-08: THE TWO-SENSE-GLOSS CLASS IS WIDER THAN THE FIVE CARDS FIXED. 04.1-07 named time, back and way; 04.1-08 fixed those plus whole and answer. But the same defect - a front offering two Spanish words behind an example that shows one - remains on others found while reading: line ('linea / fila', the example shows the drawn line and never the queue) and rate ('tarifa / tasa', the example shows the price and never the ratio). Not swept: nobody has read the other 463 cards looking for it, and there are roughly forty slash-glosses in the deck. NOW EXPENSIVE: 04.1-08's in-place exception was authorised only because the deck had never shipped, and it expires on the first deploy, after which each fix is a retirement plus a new slug. Whoever sweeps this should do it BEFORE the deck ships or accept the permanent-key cost. | open |  | 2026-08-05T06:22:52.954Z |  |
| 79 | 04.1 | deviation | src/lib/content/core-vocabulary.ts |  | 04.1-08: A FIFTH DEFECT CLASS, FOUND BY READING 37 CARDS AND NEVER LOOKED FOR ACROSS THE OTHER 463 - a gloss and an example naming DIFFERENT SPANISH VERBS. Three instances in 37: grow ('crecer' against 'She grows tomatoes' = cultivar), force ('fuerza / obligar' against 'He forced the lock' = forzar), develop ('desarrollar' against 'develop film' = revelar). TWO OF THE THREE WERE INTRODUCED BY 04.1-08'S OWN finding-1 REWRITES - a subject rewrite quietly changed the verb's argument structure and therefore its sense - and the third, develop, was PRE-EXISTING since 04.1-06 and had survived the id gate, the content harness, four near-duplicate scans and the 04.1-07 reader pass. This is the same class as 'standard', which 04.1-07 called the sharpest content defect in the phase. THE LESSON: read the FINAL TEXT of a rewritten card, not the diff - a rewrite aimed at one dimension breaks another, and no gate can see either. At three in 37 (8 percent), a sweep of the remaining 463 should be expected to find more. | open |  | 2026-08-05T06:22:53.587Z |  |
| 80 | 04.1 | deviation | scripts/verify-id-stability.mts |  | 04.1-08: THE --update REFUSAL IS CORRECT AND ITS AUTHORISED-OVERRIDE PROCEDURE IS UNDOCUMENTED. --update refuses every re-point and states there is deliberately no flag, which is right. But when a re-point IS legitimately authorised - as at 04.1-08, where three measured facts proved the deck had never shipped and no learner held any vocab: key - nothing in AGENTS.md or in any script header says what to do. The procedure used was inferred by reading verify-id-stability.mts:421-443: the refusal loop only inspects ids already present in previous.ids, so deleting the stale entries lets --update record them as new, with next.ids hashed by the script from the tree and next.retired carried over untouched. That keeps it auditable and hand-writes no hash. Recorded because the next author facing an authorised re-point will otherwise re-derive it from source, or worse, hand-edit hashes. | open |  | 2026-08-05T06:22:54.175Z |  |
...
[
  ...
  },
  {
    ...
    "resolved_at": "2026-08-05T06:22:04.883Z"
  },
  {
    "id": 72,
    "kind": "deviation",
    ...
    "resolved_at": "2026-08-05T06:22:05.395Z"
  },
  {
    "id": 73,
    "kind": "deviation",
    ...
    "resolved_at": "2026-08-05T06:22:05.983Z"
  },
  {
    "id": 74,
    "kind": "unrun-verify",
  ...
  },
  {
    ...
    "file": "src/lib/content/core-vocabulary.ts",
    "line": null,
    "description": "04.1-08: THE TWO-SENSE-GLOSS CLASS IS WIDER THAN THE FIVE CARDS FIXED. 04.1-07 named time, back and way; 04.1-08 fixed those plus whole and answer. But the same defect - a front offering two Spanish words behind an example that shows one - remains on others found while reading: line ('linea / fila', the example shows the drawn line and never the queue) and rate ('tarifa / tasa', the example shows the price and never the ratio). Not swept: nobody has read the other 463 cards looking for it, and there are roughly forty slash-glosses in the deck. NOW EXPENSIVE: 04.1-08's in-place exception was authorised only because the deck had never shipped, and it expires on the first deploy, after which each fix is a retirement plus a new slug. Whoever sweeps this should do it BEFORE the deck ships or accept the permanent-key cost.",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-08-05T06:22:52.954Z",
    "resolved_at": null
  },
  {
    "id": 79,
    ...
    "file": "src/lib/content/core-vocabulary.ts",
    "line": null,
    "description": "04.1-08: A FIFTH DEFECT CLASS, FOUND BY READING 37 CARDS AND NEVER LOOKED FOR ACROSS THE OTHER 463 - a gloss and an example naming DIFFERENT SPANISH VERBS. Three instances in 37: grow ('crecer' against 'She grows tomatoes' = cultivar), force ('fuerza / obligar' against 'He forced the lock' = forzar), develop ('desarrollar' against 'develop film' = revelar). TWO OF THE THREE WERE INTRODUCED BY 04.1-08'S OWN finding-1 REWRITES - a subject rewrite quietly changed the verb's argument structure and therefore its sense - and the third, develop, was PRE-EXISTING since 04.1-06 and had survived the id gate, the content harness, four near-duplicate scans and the 04.1-07 reader pass. This is the same class as 'standard', which 04.1-07 called the sharpest content defect in the phase. THE LESSON: read the FINAL TEXT of a rewritten card, not the diff - a rewrite aimed at one dimension breaks another, and no gate can see either. At three in 37 (8 percent), a sweep of the remaining 463 should be expected to find more.",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-08-05T06:22:53.587Z",
    "resolved_at": null
  },
  {
    "id": 80,
    "kind": "deviation",
    "phase": "04.1",
    "file": "scripts/verify-id-stability.mts",
    "line": null,
    "description": "04.1-08: THE --update REFUSAL IS CORRECT AND ITS AUTHORISED-OVERRIDE PROCEDURE IS UNDOCUMENTED. --update refuses every re-point and states there is deliberately no flag, which is right. But when a re-point IS legitimately authorised - as at 04.1-08, where three measured facts proved the deck had never shipped and no learner held any vocab: key - nothing in AGENTS.md or in any script header says what to do. The procedure used was inferred by reading verify-id-stability.mts:421-443: the refusal loop only inspects ids already present in previous.ids, so deleting the stale entries lets --update record them as new, with next.ids hashed by the script from the tree and next.retired carried over untouched. That keeps it auditable and hand-writes no hash. Recorded because the next author facing an authorised re-point will otherwise re-derive it from source, or worse, hand-edit hashes.",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-08-05T06:22:54.175Z",
    "resolved_at": null
  }
]
````
