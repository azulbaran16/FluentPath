...
| CELPIP-06 | Phase 2.1 | Pending — **the machinery exists, the bank does not** (02.1-08). The content types cover the exam's four parts (correspondence, diagram, information, viewpoints), the drop-down blank that appears in three of them is a real gradable control (`DropdownBlank`, a native `select` rendered inline in the prose), a passage-and-diagram runner grades questions and blanks through one answers map, per-question explanations are REQUIRED at the type level on both item types, and `/celpip/reading/[setId]` builds. `READING_SETS` is **empty**, so no passage, no question and no explanation has been authored and the landing correctly still reports Reading as not yet available. 02.1-09 lands the bank |
